...
- **Today** (`/today`): feeding/dosing for the current weekday plus maintenance due across tanks **and** live-food cultures. Culture tasks are tagged with the bin they apply to, or **all bins** for station-wide items
...
- **Cultures** (`/cultures`): live-food stations, **not** display tanks. One purpose per station (Daphnia *or* green water — green water is grown as feed, not fed itself). Each station has bins (lit/unlit, and optionally heated with a standing heater setpoint), a feed/look/harvest log (looks record water temperature per bin; history entries are editable; logging a feeding updates last done / next due on the logged feeding schedule, same as ✓ Fed; logging also updates that bin's hitchhikers, last look/feed/temp, and marks a crash), a recurring schedule, harvest destination (tank, another culture, or a specific bin), and harvest-status badges. Logged culture tasks also show on Today (each tagged with its bin, or all bins). Culture log writes never trigger tank AI
